  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      body: collaborators.isEmpty
          ? const EmptyState(icon: Icons.people_outline_rounded, title: 'Chưa có cộng tác viên nào')
          : ListView.separated(
              ...
              itemBuilder: (context, i) {
                ...
                final remaining = assignments.fold<num>(0, (s, a) => s + a.remainingAmount);
                return Card(
                  child: ListTile(
                    ...
                    subtitle: Text('${assignments.length} hồ sơ • Còn phải trả: ${MoneyUtils.format(remaining)}'),
                    trailing: !c.active
                        ? const Icon(Icons.pause_circle_outline_rounded, color: Colors.grey, size: 18)
                        : null,
                    onTap: () => Navigator.of(context).push(
                      MaterialPageRoute(builder: (_) => CollaboratorDetailScreen(collaboratorId: c.id)),
                    ),
                  ),
                );
              },
