...
class StatisticsScreen extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final inProgress = aggregates
        .where((a) => a.profile.status == ProfileStatus.inProgress)
        .length;
    final waiting = aggregates
        .where((a) => a.profile.status == ProfileStatus.waiting)
        .length;
    final overdue = aggregates
        .where((a) => a.deadlineCategory == DeadlineCategory.overdue)
        .length;
    final completed = aggregates
        .where((a) => a.profile.status == ProfileStatus.completed)
        .length;
    ...
    final remainingToReceive = totalAmount - received < 0
        ? 0
        : totalAmount - received;
    final commissionRemaining = commissionTotal - commissionPaid < 0
        ? 0
